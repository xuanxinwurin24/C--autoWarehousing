...
namespace CIM.Lib.Model
{
    ...
    public class Alarm
    {
        ...
        public SortedList<uint, string> Names = new SortedList<uint, string>();
        ...
        {
            if (Names.ContainsKey(Unit) == false)
            {
                Names.Add(Unit, sName_);
            }
        }
        ...
        void SetOccur(AlarmBody obj_, bool value_)
        {
            ...
            else
            {
                ...
                {
                    if (isAlarmExist() == false)
                    { bAlarming = false; bAlmFinal = true; }
                }
                else//warning disapear
                {
                    if (isWarningExist() == false)
                    { bWarming = false; bAlmFinal = true; }
                }
            }

            if (AlarmEvent != null)
            { AlarmEvent(obj_, bAlmFinal); }
        }
        public void Acknowledge(uint unit_, uint ID_)
        {
            AlarmBody obj = null;
            if (BodyList.TryGetValue((unit_ << 16) + ID_, out obj) == false)
            { return; }
            if (obj.NeedAck == true)
            {
                //obj.bOccured = false;
        ...
        public bool isAlarmExist(uint eqType_ = 0)
        {
            foreach (KeyValuePair<uint, AlarmBody> obj in BodyList)
            {
                ...
                {
                    if (eqType_ == 0 || eqType_ == obj.Value.EqType)
                    { return true; }
                }
            }
            return false;
    ...
    public class AlarmBody
    {
        ...
        [XmlIgnore]
        public string OccurTimeStr { get { return OccurTime.ToString("yyyyMMdd-HH:mm:ss:fff"); } }
        [XmlIgnore]
        public string DisOccurTimeStr { get { return DisOccurTime.ToString("yyyyMMdd-HH:mm:ss:fff"); } }

        bool fbOccured;
        [XmlIgnore]
        public bool bOccured
        {
            get { return fbOccured; }
            set
            {
